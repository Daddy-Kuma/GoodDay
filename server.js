  ...
  try {
    ...
    if (url.pathname === "/health") {
      sendJson(res, 200, {
        ...
        cacheAgeMs: cache ? Date.now() - cacheAt : null,
        upstream: "sporttery-football-crs",
        now: new Date().toISOString(),
      });
      return;
    }
    if (url.pathname === "/api/official-feed") {
    ...
    sendJson(res, 502, { error: error.message || "official feed unavailable" });
  }
});

server.listen(PORT, () => {
  console.log(`World Cup odds PWA running at http://localhost:${PORT}`);
});
